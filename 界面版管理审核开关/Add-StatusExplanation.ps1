...
# Add-StatusExplanation.ps1
# 用途：给 Win11SecurityHardeningManager_V1 界面增加“状态说明”区域，并把状态列拆成：执行结果/启用状态/详细说明
# 运行位置：Win11SecurityHardeningManager_V1 项目根目录
# 执行：powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Add-StatusExplanation.ps1

$ErrorActionPreference = 'Stop'
$main = Join-Path $PSScriptRoot 'MainForm.cs'
if (!(Test-Path $main)) { throw "找不到 MainForm.cs，请在项目根目录执行。" }
...
Write-Host "已更新 MainForm.cs：增加界面状态说明，并拆分状态列。" -ForegroundColor Green
Write-Host "请执行 dotnet clean; dotnet build 或在 VS 中清理/重新生成解决方案。" -ForegroundColor Yellow
